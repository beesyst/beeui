...
- Text search with an `id` table debounces trimmed queries of three or more characters for 275 ms, does not automatically request one or two characters, clears a previously active search, resets configurable `pagination.page_param` (default `page`) to `1`, aborts stale requests and updates URL with `history.replaceState` after a successful replacement.
