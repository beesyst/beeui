...
BeeUI owns one generic icon renderer: product supplies only a safe canonical Tabler icon name (`^[a-z][a-z0-9-]*$`). Tabs and sidebar use the locally vendored official `@tabler/icons-webfont` 3.46.0 asset through the same primitive. Raw SVG/HTML/CSS/JS, URLs and provider selection are rejected; unsafe names fail validation, while unknown safe names do not crash rendering. There is no CDN, external runtime asset or product-specific icon registry.
